...
public class Utility {
  ...
  public static String paging2(int total, int nowPage, int recordPerPage, String col, String word, int carnumber) {
    ...
    if (nowGrp < totalGrp) {
      str.append("<li><A href='/carinfo/mainlist/"+carnumber+"?col=" + col + "&word=" + word + "&nowPage=" + _nowPage + "'>다음</A></li>");
    }
    str.append("</ul>");
    str.append("</div>");
    ...
    try {
      if (filesize > 0) { // 파일이 존재한다면
        ...
        if (oldfile.exists()) {
          for (int k = 0; true; k++) {
            ...
            if (!oldfile.exists()) { // 존재하지 않는 경우
              carimage = "(" + k + ")" + originalFilename;
              break;
            }
          }
        } else {
          carimage = originalFilename;
        }
        ...
        if (os.equals("mac os x")) { // Mac
          ...
          serverFullPath = basePath + "/" + carimage;
        ...
        System.out.println("fileName: " + carimage);
