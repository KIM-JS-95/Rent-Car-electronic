...
<body>
	<div class="container">
		...
		<form action="list" class='form-inline'>
			<div class='form-group'>
				<select class='form-control' name='col'>
					<option value="mname"
						<c:if test="${col == 'mname' }">selectes</c:if>>성명</option>
					<option value="title"
						<c:if test="${col == 'title' }">selectes</c:if>>제목</option>
					<option value="content"
						<c:if test="${col == 'content' }">selectes</c:if>>내용</option>
					<option value="title_content"
						<c:if test="${col == 'title_content' }">selectes</c:if>>제목+내용</option>
					<option value="total"
						<c:if test="${col == 'total' }">selectes</c:if>>전체출력</option>
				</select>
			</div>
			<div class="form-group">
			...
			<button class='btn btn-default' type='button'
				onclick="location.href='/contents/list/create'">등록</button>
		</form>
		<table class="table table-striped">
            ...
            <c:set var="rcount" value="${util:rcount(dto.listno,rservice) }"/>
